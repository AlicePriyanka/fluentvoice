...
async function generateReport(runner) {
  ...
  addTitle(detail, "Detailed Test Results", 14);
  const detailHeader = detail.addRow([
    "Test ID", "Category", "Test Name", "Description", "Steps", "Expected", "Actual",
    "Status", "Browser", "Viewport", "Duration (ms)", "Screenshot", "Error", "Executed At",
  ]);
  styleHeader(detailHeader);
  for (const result of results) {
    const row = detail.addRow([
      result.id, result.category, result.name, result.description, result.steps, result.expected,
    ...
    row.getCell(8).fill = { type: "pattern", pattern: "solid", fgColor: { argb: statusFill(result.status) } };
    row.alignment = { vertical: "top", wrapText: true };
  }
  detail.autoFilter = { from: "A2", to: "N2" };
  detail.columns = [
    { width: 13 }, { width: 24 }, { width: 32 }, { width: 38 }, { width: 42 }, { width: 35 },
